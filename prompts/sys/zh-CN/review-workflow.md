...
- Finding 只通过 `comment` 提交，并遵循其定位与重试契约。若后续证据推翻已提交 Comment，必须调用 `retract_comment`；只在总结中声称“撤回”不会生效。只有宿主证据覆盖达到 `review_file_count` 后才调用 `task_done`，不得用它试探调查是否完整。若返回 `missing_review_files`，调查列出的每个文件后再重试。最终文本会被忽略。
