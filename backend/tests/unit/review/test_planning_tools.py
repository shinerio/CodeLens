...
from codelens.review.infrastructure.planner_output import PlannerOutputCodec, PlannerSelectionDto
...
async def test_finalize_plan_returns_success_envelope_and_rejects_repeat() -> None:
    ...
    submission = PlannerSelectionDto(
        schema_version="2",
        reviewer_references=["security:v2", "performance:v2"],
    )

    accepted = json.loads(await collector.finalize(submission))
    repeated = json.loads(await collector.finalize(submission))
...
async def test_finalize_plan_rejects_invalid_business_selection() -> None:
    ...
    result = json.loads(await collector.finalize(submission))
