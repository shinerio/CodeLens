...
def test_agent_cannot_resolve_an_unavailable_skill_policy() -> None:
    agent = replace(
        builtin_agent_catalog()["correctness:v1"],
        skill_policy_ref="unavailable:v1",
    )
...
def test_resolver_returns_a_deterministic_frozen_spec() -> None:
    ...
    assert first.skill_policy.reference == "none:v1"
    assert first.skills == ()
...
def test_resolver_activates_only_skills_from_the_static_policy() -> None:
    ...
    agent = replace(
        builtin_agent_catalog()["security:v1"],
