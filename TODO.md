...
- review详情页面支持导出功能，生成结构化的review意见，包括代码片段，old或者new，行号，commit_id，review意见，置信度，意见，问题等等所有详细信息。导出为json格式。用于指导其他
  agent进行修复。注意：导出功能要可扩展，支持将review意见对接到后续的各种平台上。
- 移除对git未提交代码review的支持，保留全仓代码review的能力。代码量比较大的时候支持分片并发进行。
- 每个工具的默认限制情况需要再梳理一下，支持settings可配。settings支持一键重置所有未默认值。工具的tools.json中考虑移除参数描述，或者至少移除数量描述或者支持动态注入
