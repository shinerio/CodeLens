...
- `GET/PUT /api/settings/tool-limits` 读取或更新工具级资源限制，字段包括 `max_results`（find_files/grep/get_diff 单页结果上限，默认 200）、`max_read_bytes`（read_file/get_diff 输出上限，默认 65536）、`max_scan_bytes`（grep 扫描上限，默认 1048576）、`max_source_bytes`（源文件大小上限，默认 1048576）、`max_file_payload_cache_bytes`（单个 Agent Run 内文件内容缓存总字节上限，超出按最久未用驱逐，默认 33554432）、`max_lines`（read_file 行数上限，默认 1000）、`max_path_chars`（路径长度上限，默认 1024）、`max_pattern_chars`（模式长度上限，默认 512）、`regex_timeout_seconds`（正则超时，默认 30.0）、`comment_batch_size`（评论批次上限，默认 20）、`short_text_max`（短文本上限，默认 240）、`long_text_max`（长文本上限，默认 8000）、`task_summary_max`（任务摘要上限，默认 8000）。PUT 支持部分更新，省略字段保留当前值。更新必须原子持久化；每个 Agent Run 开始时读取当前值，无需重启即可生效。
- `GET/PUT /api/settings/node-limits` 读取或更新进程级资源限制（内存上限、内存压力阈值、并发 Review/Agent 数），持久化到 `data/node-settings.json`。这些值在 Worker 进程启动时一次性注入 `MemoryGuard` 和并发信号量，**修改后需重启进程生效**，运行期不可热更新。首次启动未持久化时以 `CODELENS_` 环境变量为初始值；持久化后文件值优先于环境变量默认值。
- `POST /api/settings/reset-all` 将所有用户可配置设置重置为产品默认值，包括工具限制、节点资源限制、执行限制、规则文件限制、Review 完成重试次数、最近仓库数量和日志级别。不影响网关身份/凭证、Agent prompt 自定义（含 Planner 与 Verifier 专属策略）和触发器插件配置。返回重置后的所有设置快照。
- `GET /api/agent-prompts` 列出所有 prompt 可编辑的内置 Agent（含 Planner 与 Verifier 等 internal DAG 角色，带 `role` 字段，不过滤 `is_public`）；`GET/PUT/DELETE /api/agent-prompts/{agent_id}` 按 `locale` 与 `version` 读取、覆盖或重置 `prompts/<prompt_key>/<locale>.md` 对应的 Agent 专属策略。覆盖按 canonical reference 与 locale 独立持久化，默认值不可变；`reviewer-catalog` 仍只返回公开 Reviewer，两者语义分离，Planner 与 Verifier 不得出现在公开 Catalog。
...
- `findings`：Finding、Evidence、校验、去重、修复检测、抑制和报告。
...
Fixed 选择由宿主直接编译为不可变 Review Plan，绝不调用 Planner；Adaptive 选择只允许 `review-planner:v2` 通过一次 `finalize_plan:v2` 提交完整选择。合法选择必须是单独的 General，或至少一个不含 General 的专项 Reviewer；所有引用都必须来自冻结 Catalog 中 Ready、公开且 Planner eligible 的 Reviewer，但不要求覆盖全部 eligible Reviewer。`correctness:v2` 不在 Planner 可选范围内，由 `ReviewPlanCompiler` 在 Planner 选择后确定性注入：当 Planner 未选择 General 时框架自动追加 `correctness:v2`，保证正确性维度始终覆盖。宿主完整性约束允许框架追加 Mandatory Reviewer，但不允许移除 Planner 已选择的 Reviewer 或追加非 Mandatory Reviewer。Planner 不能提交工具、能力、Finding、Prompt 或自由 DAG；Reviewer、Verifier 和 Deduplicator 的节点及依赖全部由宿主编译。每个计划始终包含一个批量 Deduplicator 节点，依赖 Verifier（存在时）或全部 Reviewer 节点；当 `existing_findings` 非空时还包含一个批量 Remediator 节点，无依赖，与 Reviewer 并行运行，使用冻结的 `existing_findings` 检测已报告问题是否已被当前变更修复。General 不包含 Verifier；Fixed 单 Specialist 不包含 Verifier；Adaptive 选择经框架注入 correctness 后至少包含两个 Reviewer（General 除外），必须预建批量 Verifier。计划在任何 Reviewer 扇出前持久化；Plan 哈希覆盖选择、节点、Planner 指引与能力降级，任务 planning context 另行哈希覆盖解析后限制。可选 MCP/Skill 不可用只形成降级元数据，不改变任务完成状态。

多 Agent 执行的就绪判断、失败归约和重启恢复只能读取持久化 Plan 与 AgentRun checkpoint，不能以进程内 `gather` 返回值作为阶段事实。Reviewer 节点相互隔离：至少一个成功时允许 Verifier 和 Deduplicator 在全部 Reviewer 终态后继续，任一 Reviewer 失败或超时设置不可被后续成功阶段清除的 sticky partial 标记；全部 Reviewer 失败时跳过未运行的下游节点并使任务失败。Deduplicator 与 Verifier 共享同一就绪规则：依赖全部终态且至少一个成功。Remediator 无 DAG 依赖，与 Reviewer 并行执行；Orchestrator 在发布前（Deduplicator 之后）必须确认 Remediator 终态，失败时标记 partial。General 与 Fixed 单 Reviewer 失败直接使任务失败。取消必须传播到全部非终态节点，已经持久化的模型输出在普通重启后保留并从验证边界继续。Worker 以全局 Agent、模型和工具信号量、每任务信号量及为并发任务保留容量的公平上限共同约束模型调用，单个 Deep Review 不得占满可并行 Worker 的全部 Agent 槽位。
...
Deduplicator 是 Verifier 之后（无 Verifier 时为单 Reviewer 之后）的最终发布前去重门，每个任务至多运行一个 `review-deduplicator:v2`（prompt key 为 `review-dedup`）batch 节点。它是 codeless Agent——不拥有证据工具，只有 `deduplicate:v2`（批量 accept/deny）和 `deduplicate_done:v2`（覆盖门）两个工具。Deduplicator 的统一标识是 `verdict_decision_id`（`"verdict_" + sha256(f"{task_id}\0{','.join(cluster_ids)}")`），每个存活的 ACCEPT 或 MERGE Verdict 在持久化时已获得该标识。发布前采用两层过滤：第一层确定性预过滤在 LLM 前运行，当存活 Finding 与已有 Finding 共享 path、category 且行范围重叠时自动 deny；第二层 LLM Deduplicator 对剩余存活 Finding 逐条判断是否与已有 Finding 重复——重复指同一问题出现在相同或行号偏移后的位置，行号跨 review 可能漂移，代码内容与问题主题匹配即为重复。不确定时 accept，错误 deny 会压制真实问题。Deduplicator 只在 `existing_findings` 非空且存在存活 Finding 时运行；两个条件之一不满足时节点被跳过，直接发布。Deduplicator 失败时确定性预过滤结果仍保留，任务标记 partial。`DedupDecision`（`verdict_decision_id`、`outcome=accept|deny`、`decision_source=deterministic|llm`）持久化到 `dedup_decisions` 表，每个 `verdict_decision_id` 每任务唯一。发布时从 Verdict 列表中过滤掉 deny 的 `verdict_decision_id`，只发布 accept 的 Finding。

Remediator 是与 Reviewer 并行的修复检测门，每个任务至多运行一个 `review-remediator:v2`（prompt key 为 `review-remediation`）batch 节点，无 DAG 依赖。它拥有证据工具（`find_files`、`grep`、`read_file`、`get_diff`）以检查当前代码，并有 `resolved_review:v2`（批量标记）和 `remediation_done:v2`（覆盖门）两个输出工具。Remediator 的统一标识是 `remediation_ref`（`"{source_id}:{finding_id}"`），每个已有 Finding 在任务冻结时已获得该标识。采用两层过滤：第一层确定性预过滤在 LLM 前运行，文件未变更的 Finding 自动标记 `unresolved`，文件已删除的自动标记 `resolved`；第二层 LLM Remediator 对剩余 Finding 逐条检查当前代码是否已修复该问题——比较 `existing_code` 锚点与当前 Snapshot 代码，保守原则：不确定时标记 `unclear`。Remediator 只在 `existing_findings` 非空时运行；全仓 Review（无 diff 基线）或无已有问题时节点被跳过，直接发布。Remediator 失败时确定性预过滤结果仍保留，任务标记 partial。`RemediationDecision`（`source_id`、`finding_id`、`outcome=resolved|unresolved|unclear`、`evidence_summary`、`decision_source=deterministic|llm`）持久化到 `remediation_decisions` 表，每个 `(source_id, finding_id)` 每任务唯一。导出信封 `FindingExportEnvelopeV2` 携带可选 `remediation` 摘要，包含已修复、未修复和待确认计数及每条决策；本地插件在 Markdown 报告中展示已修复和待确认问题，并在回读时跳过 `resolved` 的已有问题形成闭环。CodeHub 插件对 `resolved` 的 CodeHub 讨论调用 `ch mr review resolve` 解决，对 `unclear` 的发布需要手动验证的评论。
...
| `review-deduplicator:v2` | — | `deduplicate:v2`、`deduplicate_done:v2` |
| `review-remediator:v2` | `find_files:v2`、`grep:v2`、`read_file:v2`、`get_diff:v2` | `resolved_review:v2`、`remediation_done:v2` |
