...
| `finalize_plan:v2` | 完成 Planner | 一次提交并校验 General 单独或至少两个专项 Reviewer 的完整选择 |
...
批量 Verdict 不产生隐式合并。Merge 的内容、类别、严重级别、维度、证据强度和位置以 Verifier 提交为准，但必须由参与 Cluster 的证据支持。Cluster/Candidate/Reviewer 来源、行号、excerpt hash、Finding ID 和 fingerprint 始终由宿主产生。
