...
每个匹配包含仓库相对路径、从 1 开始的行号和最多 200 字符的该行文本。工具最多扫描 1 MiB 内容、返回 200 个匹配；任一上限触发时 `truncated` 为 `true`。含 NUL 字节的二进制条目会被跳过。

### 4.4 工作原理

1. 在主进程中编译正则，提前拒绝非法表达式。
2. 按路径顺序逐一读取可见条目；每次读取都复验 Manifest 哈希，二进制内容不参与搜索。
3. 将不超过扫描上限的文本交给使用 `spawn` 创建的独立子进程。
4. 子进程逐行执行正则搜索并收集有界结果。默认计算时限为 5 秒；超时后宿主会终止该进程，必要时再强制杀死，避免灾难性回溯长期占用 Runtime。

`grep` 的范围是所有可见条目，当前契约不提供目录或文件过滤参数。需要缩小范围时，可先用 `find_files` 定位路径，再用 `read_file` 精读。
