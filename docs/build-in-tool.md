...
`FilesystemReviewTools` 本身不为 4 个证据工具设置独立调用次数上限。生产 Runtime 会在更外层用同一个 `ToolExecutionLimiter` 包装全部 7 个工具，共享当前模型网关配置的最大工具调用数、单次工具超时和相同参数与结果的重复熔断阈值；调查还受 Agent 最大模型回合数、单次工具输出上限和用户取消约束。`grep` 的隔离进程启动阶段单独受限，只有 Worker 就绪后才开始计算正则求值时限，避免把解释器冷启动误判为表达式超时。每次模型可见工具调用及结果会以 `tool_call`/`tool_result` 事件进入运行转录，并用于过程报告中的工具调用统计。
...
4. 应用 64 KiB 输出上限、UTF-8 替换解码和行号前缀。全文模式返回实际截取范围；显式范围保留请求的起止行号，即使结束行超过 EOF；同时返回截断状态。
