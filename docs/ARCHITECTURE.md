...
- `POST /api/reviews` 接受且仅接受 v2 `reviewer_selection` 或兼容字段 `selected_agents` 之一。v2 selection 使用 `fixed`（带 1–32 个不可重复版本引用）或 `adaptive` 判别联合，并同时冻结可选的 profile ID/revision；legacy 字段只能映射为 Fixed + Standard，不能隐式升级 Reviewer。Review 读取响应稳定返回原始 `selection_request`、profile 来源、可空 `review_plan`、按 Planned/Completed/Failed/Omitted 重建的 Reviewer coverage、Verdict 计数（`verdict_summary` 含 accept/deny/merge 三态），以及实际 Plan Reviewer 的 `selected_agents` 兼容投影；这些字段只能从持久化 Plan、checkpoint 和决策重建，SSE 不是真相来源。
- `GET /api/reviewer-catalog` 只返回公开且不可变的 Reviewer 版本，包含维度、成本类别、Planner eligibility、Capability readiness 和 legacy 可见性；Planner、Verifier 与隐藏 legacy Reviewer 不得出现在公开 Catalog。
...
- `/api/plugins` 是统一插件集合契约：`GET` 列出所有已安装插件，`GET /{plugin_id}` 返回显式的 API 兼容状态、配置修订和复制策略，`POST /install` 接收 `{git_url, ref?}` 并把外部 Git 插件安装到 `data/plugins/{plugin_id}/`，`DELETE /{plugin_id}` 卸载外部插件；`PUT /{plugin_id}/trigger/enable|disable|config` 和 `PUT /{plugin_id}/report/enable|disable|config|auto-export` 分别管理 Trigger 与 Report 能力。未声明 `plugin_api_version` 的 Manifest 明确视为 v1；v2 插件必须使用主版本 2 以上并声明兼容的 `min_codelens_version`，候选代码必须在激活或加载前再次通过兼容检查。配置写入前必须同时匹配 Manifest Schema 和 Core v2 Reviewer Policy 不变量，未声明的能力必须拒绝；本地 Git Hook 通过 `/trigger/install-hooks`、`/trigger/uninstall-hooks` 和 `/trigger/hook-status` 显式同步与查询。统一记录持久化到 `data/plugins.json`，写入必须原子化并串行保护读改写；插件更新时，候选代码与迁移后配置作为一个用户可见事务激活，任一失败恢复旧目录和旧记录。内置插件 ID `local` 为保留 ID 且不可卸载。外部入口代码通过受控 loader 加载，实现对应 API 版本的 `TriggerSinkPort` 或 `ReportSinkPort`，卸载和重新安装必须使旧实例缓存失效。安装请求和普通运行日志不得记录可能携带凭证的完整 Git URL。
...
Comment v2 Reviewer 输出首先形成与 task、AgentRun、Snapshot、Reviewer 和证据哈希绑定的 Candidate 审计记录，不能直接成为公开 Finding。宿主必须重新校验规范路径、侧别、变更 hunk、摘录哈希、维度和证据身份，再按位置、类别、标题、影响和证据的规范化键确定性聚类，生成携带 canonical 字段的 `FindingCluster`。多 Specialist 的 Verifier 只接收按 Plan hash 稳定打散、且移除 Reviewer/Run 执行顺序的 Candidate 投影与 Cluster 上下文；它必须通过 `verdict`（accept/deny）或 `merge` 工具对每个 Cluster 恰好提交一次决策，并通过 `finalize_verdicts` 校验全部 Cluster 被覆盖且无重复。accept 使用 Cluster 的 canonical 字段物化 Finding；merge 使用模型合成的全字段物化 Finding；deny 抑制。不得引用 Cluster 外 Candidate、发明证据或把严重级别提高到所有合并 Candidate 之上。Cluster、Candidate 与 Verdict 决策必须作为审计状态持久化，Verdict 决策和节点成功在同一事务提交。General 与 Fixed 单 Reviewer 不运行 Verifier，仅允许 direct evidence 且影响为 confirmed 或 plausible 的 Candidate 进入直接发布决策，其余 Candidate 只保留为 deny 审计记录。

Verifier 在多 Specialist 场景下运行，并且每个任务至多运行一个 `review-verifier:v1`（prompt key 为 `review-verdict`）batch 节点；输入覆盖全部 Cluster。模型通过 `verdict` 工具提交 accept 或 deny，通过 `merge` 工具提交合成字段（全部必填），通过 `finalize_verdicts` 校验覆盖完整性。只有 accept 和 merge 的规范表示可生成最终 Finding；deny 保持抑制。Comment v2 Finding 的 confidence 必须为 null，并保留分类轴、证据强度、影响确定性、可复现性和来源 Reviewer 引用；Comment v1 的数值 confidence 与既有字段保持兼容。最终 Finding 插入、Verdict 状态与发布事件必须在一个幂等事务中提交，并继续依赖 `(task_id, fingerprint)` 唯一效应保证重放不会重复发布。Verifier 失败不得撤回已经满足直接发布条件的 Finding，但任务必须保留 partial 状态。
...
| `review-verifier:v1` | `read_file`、`get_diff` | `verdict`、`merge`、`finalize_verdicts` |
