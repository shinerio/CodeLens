...
- 统一文件排除策略由项目根目录 `conf/file-exclusions.toml` 提供，包含默认开启的 `exclude_binary`、后缀列表 `suffixes` 和仓库相对 POSIX 路径正则列表 `path_regexes`。该配置是本机运行时的受信任操作员输入，不通过 HTTP 或 Web 设置暴露；启动时必须严格校验完整字段、类型与正则。创建 Review 时重新读取并把当前策略的 canonical JSON 与哈希冻结到任务，已创建任务不得因后续配置变化而改变文件范围。
