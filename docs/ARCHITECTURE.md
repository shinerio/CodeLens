...
- 统一文件排除策略由项目根目录 `conf/file-exclusions.toml` 的基础策略与 `GET/PUT /api/settings/file-exclusions` 管理的 Web 自定义层合并而成，两者都包含 `exclude_binary`、后缀列表 `suffixes` 和仓库相对 POSIX 路径正则列表 `path_regexes`。后缀与正则取规范化去重后的并集，`exclude_binary` 在任一来源开启时开启。TOML 在启动时必须严格校验；创建 Review 时必须重新读取两个来源，使配置修改无需重启即对后续任务生效。当前有效策略的 canonical JSON 与哈希冻结到任务，已创建任务不得因后续配置变化而改变文件范围。
