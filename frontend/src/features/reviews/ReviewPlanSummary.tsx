import type { BudgetProfile, ReviewPlanProjection, ReviewerSelection } from "./types";
...
export function ReviewPlanSummary({ selection, plan, budget }: { selection: ReviewerSelection; plan: ReviewPlanProjection | null; budget: BudgetProfile }) {
  const { t } = useI18n();
  ...
  return (
    <section className="review-plan-summary" aria-label={t("plan.label")}>
      ...
      <div><span>{t("plan.budget")}</span><strong>{t(BUDGET_LABELS[budget])}</strong></div>
